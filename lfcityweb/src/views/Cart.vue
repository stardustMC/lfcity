    ...
    <div class="cart-main">
      <div class="cart-header">
        <div class="cart-header-warp">
          <div class="cart-title left">
            ...
            <div class="left">
              共<span>{{cart.course_list.length}}</span>门，已选择<span>{{cart.selected_course_list.length}}</span>门
            </div>
          </div>
          <div class="right">
            <div class="">
      ...
      <div class="cart-body" id="cartBody">
        ...
        <div class="cart-body-table">
          <div class="item" v-for="course in cart.course_list">
              <div class="item-1">
                  <el-checkbox :true-value="1" :false-value="0" v-model="course.selected" @change="(value)=>alter_course_select(course.id, value)"></el-checkbox>
              </div>
              <div class="item-2">
                  <router-link :to="{name: 'Detail', params: {id: course.id}}" class="img-box l">
                    <img :src="course.course_cover" alt="">
                  </router-link>
                  <dl class="l has-package">
                    <dt>[{{course.course_type}}]-{{course.name}}</dt>
                    <p class="package-item" v-if="course.discount.price">优惠价</p>
                  </dl>
              </div>
              <div class="item-3">
                  <div class="price">
                      <span class="discount-price" v-if="course.discount.price"><em>￥</em><span>{{course.discount.price}}</span></span><br>
                      <span :class="{'original-price':course.discount.price}"><em>￥</em><span>{{course.price}}</span></span>
                  </div>
              </div>
              <div class="item-4">
                <el-icon :size="26" class="close" @click="delete_course(course.id)"><Close /></el-icon>
              </div>
          </div>
...
const delete_course = (course_id)=>{
  let token = localStorage.getItem("token") || sessionStorage.getItem("token");
  cart.delete_course(token, course_id).then(response=>{
    // 移除被删除的课程
    let index = cart.course_list.findIndex(course => course.id === course_id);
    cart.course_list.splice(index, 1);
    // 更新购物车内课程数量
